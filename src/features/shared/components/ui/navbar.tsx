...
export function Navbar() {
  ...
  // Cleanup mobile menu when authentication state changes
  useEffect(() => {
    if (!isAuthenticated) {
      setIsMobileMenuOpen(false)
      document.body.style.overflow = 'unset'
      // Force cleanup with a small delay to ensure proper state reset
  ...
  return (
    <header className={headerClasses}>
      <nav className="container mx-auto px-4 md:px-8">
        ...
        {isMobileMenuOpen && (
          <div
            className="md:hidden fixed inset-0 bg-black/60 transition-all duration-300 z-40"
        ...
        {isMobileMenuOpen && (
          <div
            className="md:hidden fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-slate-50 shadow-2xl border-l border-slate-200 transform transition-all duration-300 ease-out z-50 translate-x-0"
          >
